...
const SCENE_SEED = [
  ...
  ['communication', '寻人/寻物', 'da_an', '大吉', '线索稳，方向不乱，适合按已有脉络去找。', ['先找熟悉地点', '问固定联系人', '顺着已知线索追'], ['临时乱换方向', '情绪化乱找'], '寻人寻物落大安，线索偏稳，宜沿原路原点细找。', 1],
  ['communication', '寻人/寻物', 'liu_lian', '小凶', '线索拖，消息慢，容易绕圈子。', ['放慢节奏复盘', '补全遗漏信息', '分段回查'], ['急着满城乱找', '只盯一个方向'], '寻人寻物落留连，线索偏拖，宜回查补漏，不宜乱追。', 2],
  ['communication', '寻人/寻物', 'su_xi', '中吉', '容易有新消息、新回音，适合马上追线索。', ['立刻联系', '立刻核实线索', '顺着新回音追'], ['拖着不核实'], '寻人寻物落速喜，眼前易有回音，宜立刻追线索。', 3],
  ['communication', '寻人/寻物', 'chi_kou', '中凶', '最怕信息失真、口误误导，容易被错线索带偏。', ['先核实来源', '只信可验证信息', '减少情绪争执'], ['听风就是雨', '因着急跟人起冲突'], '寻人寻物落赤口，最怕错信消息，宜先核实再动。', 4],
  ['communication', '寻人/寻物', 'xiao_ji', '小吉', '线索不算大开，但细找细问会有收获。', ['小范围细找', '轻问轻查', '优先近处'], ['一下子铺太大'], '寻人寻物落小吉，宜近处细查，往往小线索先露头。', 5],
  ['communication', '寻人/寻物', 'kong_wang', '大凶', '线索偏空，容易扑空、白找、白等。', ['先停一下重整线索', '确认最后出现点', '减少空耗'], ['盲找', '硬耗时间体力'], '寻人寻物落空亡，眼前易扑空，宜先收拢线索再追。', 6],
  ['travel', '健康/出行', 'da_an', '大吉', '状态较稳，出行可行，按计划安排多半顺。', ['照计划出门', '保持规律作息', '稳步办事'], ['临时大改节奏'], '健康出行落大安，身心与行程都偏稳，宜按计划行动。', 1],
  ['travel', '健康/出行', 'liu_lian', '小凶', '状态恢复慢，路上也易拖延，宜放缓。', ['预留时间', '减少行程强度', '先顾体力'], ['赶路过急', '硬撑出门'], '健康出行落留连，节奏偏慢，宜缓行缓办，先顾状态。', 2],
  ['travel', '健康/出行', 'su_xi', '中吉', '适合快办快回，利短程、利当面处理。', ['抓窗口出门', '重要事快办', '见面快处理'], ['拖沓磨蹭'], '健康出行落速喜，适合快进快出，利把事迅速办成。', 3],
  ['travel', '健康/出行', 'chi_kou', '中凶', '最怕路上烦躁、现场摩擦，身心容易被火气带乱。', ['稳住情绪', '放慢表达', '减少硬碰硬'], ['带情绪出门', '临场争执'], '健康出行落赤口，宜稳情绪稳节奏，别让火气坏事。', 4],
  ['travel', '健康/出行', 'xiao_ji', '小吉', '适合日常出行、轻量奔波，普通安排多能顺手。', ['轻装出门', '办小事', '日常检查'], ['把普通顺势当成重局'], '健康出行落小吉，适合轻行轻办，小事更顺。', 5],
  ['travel', '健康/出行', 'kong_wang', '大凶', '容易白跑或体力透支，眼前不宜硬上强度。', ['先确认再动', '能缓则缓', '先顾身体'], ['强撑出行', '临时加码行程'], '健康出行落空亡，宜先顾身与现实安排，不宜硬赶。', 6],
...
function detectSceneType(question = '', preferredScene = '') {
  ...
  if (/(寻人|找人|失联|联系不上|寻物|找回|遗失|丢了|丢失|线索|下落)/.test(text)) return 'communication';
  if (/(健康|身体|不舒服|难受|恢复|出门|出行|拜访|见面|约见|路上|行程)/.test(text)) return 'travel';
  return 'decision';
}
...
function pickLikelyConcern(sceneType) {
  const mapping = {
    ...
    communication: '这件事更像是在问：人或物的线索眼前够不够实，现在该往哪边追更有效。',
    travel: '这件事更像是在问：这次出行和身体状态能不能承住，值不值得现在动。',
