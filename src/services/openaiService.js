...
function normalizeMingSkyNarrativeSection(section, index, fallbackEvidenceRefs = []) {
  ...
  return {
    section_id: textOf(section?.section_id || section?.id, `section_${index + 1}`)
      .toLowerCase()
      .replace(/[^a-z0-9_]+/g, '_')
      .replace(/^_+|_+$/g, ''),
...
async function runMingSkyNarrative({ payload, model = DEFAULT_MODEL }) {
  ensureOpenAIKey();
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a structured narrative rendering model for MingSky Astrology. Output valid JSON only.',
      },
      {
        role: 'user',
